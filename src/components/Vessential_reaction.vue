        ...
        <div id="user_choose_module" style="width: 95%;margin: auto;margin-top: 10%;">
          ...
          <div style="margin-top: 20px;width: 100%;">
            <el-form :model="select_form" ref="select_form"  class="demo-ruleForm">
              ...
              <el-form-item  label="敲除反应" prop="delete_reaction" class="item_style labelsize" required="required" style="width: 30%;float: left">
                  <el-select v-model="select_form.delete_reaction.value" multiple filterable placeholder="请选择" style="float: left">
                      <el-option v-for="item in select_form.delete_reaction" :key="item.value" :label="item.value" :value="item.value"></el-option>
                  </el-select>
              </el-form-item>
          ...
          <!--影子价格结果展示部分-->
          <el-table :data="tableData" style="width: 100%;margin-bottom: 30px;" :default-sort = "{prop: 'date', order: 'descending'}" border>
              <el-table-column label="敲除反应" minWidth="7%">
                  <template slot-scope="scope">
                      <span>{{scope.row.reaction_name}}</span>
                  </template>
              </el-table-column>
              <el-table-column label="敲除前最大通量" minWidth="7%">
                  <template slot-scope="scope">
                      <span>{{scope.row.before_knockout}}</span>
                  </template>
              </el-table-column>
              <el-table-column label="敲除后最大通量" minWidth="10%">
                  <template slot-scope="scope">
                      <span>{{scope.row.after_knockout}}</span>
                  </template>
              </el-table-column>
              <el-table-column label="敲除后通量降低" prop="knockout_rate" sortable minWidth="10%">
                  <template slot-scope="scope">
                      <span>{{scope.row.knockout_rate}}</span>
                  </template>
              </el-table-column>
          </el-table>
        </div>
    ...
    methods: {
          ...
          this.$axios.request(
            {
              ...
              if(result.data.code===1000){
                  ...
                  that.select_form.delete_reaction = {...result.data.debug_reaction};
              }
              else{
      ...
      submitParams(){
        var that=this;
          this.$axios.request(
            {
              url:'http://127.0.0.1:8000/cobrapy_function/essential_reaction/',
              method:'POST',
              data:{
                  bacterial: that.select_form.bacterial.value,
                  biomass_reaction: that.select_form.biomass_reaction.value,
                  target_reaction: that.select_form.target_reaction.value,
                  delete_reaction: that.select_form.delete_reaction.value,
